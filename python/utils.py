...
def read_sql_query_fallback(max_lookback: int = 1, **kwargs: dict) -> pd.DataFrame:
    ...
    specified. Note this function is specific to querying ACS Tables, LEHD LODES,
    EDD point-level, and QCEW data. This function requires the SQL query file to return
    a DataFrame with a single column called 'msg' with one of 6 text strings: 'ACS
    5-Year Table does not exist', 'ACS 1-Year Table does not exist', 'PUMS 5-Year does
    not exist', 'LODES data does not exist', 'EDD point-level data does not exist', or
    'QCEW data does not exist' when no data is found for the specified year.
